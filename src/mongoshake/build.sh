...
for g in "${goos[@]}"; do
    ...
    for i in "${modules[@]}" ; do
        ...
        # build
        if [ $DEBUG -eq 1 ]; then
            $run_builder ${compile_line} -ldflags "-X $build_info" -gcflags='-N -l' -o "$output/$i.$g" -tags "debug" $all_files
        else
            $run_builder ${compile_line} -ldflags "-X $build_info" -o "$output/$i.$g" $all_files
        fi
